...
if ( isset( $_POST['coltpay_nonce'] ) && wp_verify_nonce( $_POST['coltpay_nonce'], 'save_api_key' ) ) {
    //Form data sent
    $coltpay_public_api_key = sanitize_text_field($_POST['coltpay_public_api_key']);
    update_option('coltpay_public_api_key', $coltpay_public_api_key);

	$coltpay_private_api_key = sanitize_text_field($_POST['coltpay_private_api_key']);
    update_option('coltpay_private_api_key', $coltpay_private_api_key);
...
<div class="wrap rentmy-admin-wrap">
    ...
    <div class="card rentmy-admin-header">
        <div class="rentmy-admin-pull-left">
            <img src="<?php echo esc_url(plugins_url('../assets/logo.png', __FILE__)); ?>" width="200"/>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <form name="rentmy_form" method="post" action="<?php echo str_replace('%7E', '~', $_SERVER['REQUEST_URI']); ?>">
            <table class="form-table">
                ...
                <tr>
                    <th scope="row">
                        <label for="coltpay_private_api_key"><?php _e("Your ColtPay Public API Key", 'coltpay'); ?></label>
                    </th>
                </tr>
                <tr>
                    <td>
                        <input type="text" name="coltpay_public_api_key" class="regular-text" value="<?php echo $coltpay_public_api_key; ?>"/>
                        <?php wp_nonce_field( 'save_api_key', 'coltpay_nonce' ); ?>
                        <p class="description"><?php _e("You can find your <b>API Key</b> under <b>Account > Account Profile >API Keys</b> in your ColtPay account.", 'coltpay'); ?></p>
                    </td>
                </tr>

				                <tr>
                    <th scope="row">
                        <label for="coltpay_private_api_key"><?php _e("Your ColtPay Private API Key", 'coltpay'); ?></label>
                    </th>
                </tr>
                <tr>
                    <td>
                        <input type="text" name="coltpay_private_api_key" class="regular-text" value="<?php echo $coltpay_private_api_key; ?>"/>
                        <?php wp_nonce_field( 'save_api_key', 'coltpay_nonce' ); ?>
                    </td>
                </tr>
</tbody>
